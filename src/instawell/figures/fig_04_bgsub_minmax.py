...
def bgsub_minmax_figure_generator(ctx: ExperimentContext):
    """
    ...
    bg_min_max_data_path = ctx.experiment_dir / StepFiles.MIN_MAX_SCALED_DATA
    if not bg_min_max_data_path.exists():
        raise FileNotFoundError(
            f"BG subtracted min-max data file not found: {bg_min_max_data_path}"
        )
